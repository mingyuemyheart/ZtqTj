...
public class SolarTermsUtil {
     ...
     * @return
     */
    public String getSolartermsNameNew() {
        String str = "";
        int sectonal = sectionalTerm(gregorianYear, gregorianMonth);
        int principle = principleTerm(gregorianYear, gregorianMonth);
        if(gregorianDate < sectonal) {
            if(gregorianMonth-2 < 0) {
                str = principleTermNames[principleTermNames.length - 1];
            } else {
                str = principleTermNames[gregorianMonth - 2];
            }
        } else if (gregorianDate >= sectonal && gregorianDate < principle) {
            str = sectionalTermNames[gregorianMonth - 1];
        } else if (gregorianDate >= principle) {
            str = principleTermNames[gregorianMonth - 1];
        }
        return str;
    }
