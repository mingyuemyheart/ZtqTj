...
public class ActivityProgramerManager extends FragmentActivityZtqBase {
	...
	private void initData() {
		...
		for (int i = 0; i < product_list.length; i++) {
			listdata.add(product_list[i]);
		}
		for (int j=0;j<product_analysis.length;j++){
            listdata.add(product_analysis[j]);
        }

		adapter = new AdapterProjectManagemer(ActivityProgramerManager.this,
				listdata);
	...
	OnItemClickListener onItemClickListener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			boolean boolean1 = !LocalDataHelper.getProductValue(
					ActivityProgramerManager.this, itemName);

			LocalDataHelper.saveProductValue(ActivityProgramerManager.this,
					itemName, boolean1);
