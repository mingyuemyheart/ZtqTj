...
public class LiveQueryViewOld0713 extends View {
    ...
    public LiveQueryViewOld0713(Context context, AttributeSet a) {
        super(context, a);
        this.setPadding(0, 0, 0, 0);
        initPointValue();
    }
    ...
    private void initPointValue() {
        ...
        int sizeText = ScreenUtil.dip2px(getContext(), 9);
        ringR = ScreenUtil.dip2px(getContext(), 4);
        ptTextButtom = DrawViewTool.getInstance().getTextPaint(getContext().getResources().getColor(R.color.livequery_actual));
        ptTextTop = DrawViewTool.getInstance().getTextPaint(getContext().getResources().getColor(R.color.livequery_prediction));
        ptTextYValue = DrawViewTool.getInstance().getTextPaint(getContext().getResources().getColor(R.color.text_black));
...
//                }
    ...
    public void setItemName(String strValue, ColumnCategory cutType, String strUnit) {
...
//        绘图高度的起始点
    ...
    private void compleValuePopupWindow(int touchX, int touchY) {
//        rectangleAcHight
//        绘图高度的起始点
        if (widthScale == 2) {
            float viewYHight = getHeight() - margButton;
            for (int i = 0; i < rectanglePreHight.size(); i++) {
                ...
                if (touchX > pX - ringR && touchX < pX + ringR) {
                    if (touchY > (pY - ringR) && touchY < (pY + ringR)) {
                        if (clicklistener != null) {
                            clicklistener.positionListener(p.x + whatX, (int) (p.y + margButton), ybList.get(i).fulldt + "\n" + strValue + ybList.get(i).val + strUnit, true);
                        }
                        return;
                    }
                }
            ...
            for (int i = 0; i < rectangleAcHight.size(); i++) {
                ...
                if (touchX > pX - xSaction && touchX < pX + xSaction) {
                    if (touchY > pY && touchY < viewYHight) {
                        if (clicklistener != null) {
                            clicklistener.positionListener(p.x + whatX, (int) (p.y + margButton), skList.get(i).fulldt + "\n" + strValue + skList.get(i).val + strUnit, false);
                        }
                        return;
                    }
                }
...
//        dstRect = new RectF(left, top, right, bottom);
        ...
        if (cutClickBitmap.bm != null) {
            ...
            canvas.drawBitmap(cutClickBitmap.bm, x, y, new Paint());
        }
    }
    ...
    public void setDoubleWidth(boolean isDouble) {
        if (isDouble) {
            cutClickBitmap.bm = null;
            widthScale = 2;
            int sizeText = ScreenUtil.dip2px(getContext(), 14);
            ptTextButtom.setTextSize(sizeText);
            ptTextTop.setTextSize(sizeText);
            ptTextYValue.setTextSize(sizeText);
    ...
    private Bitmap drawImage() {
        ...
        margButton = (int) (3* fontHeight);
        margTop = (int) (2* fontHeight);

        margLeft = 3 * fontHeight;
        if (widthScale == 2) {
            margRight = 3 * fontHeight;
        } else {
        ...
        canvasBm.drawLine(margLeft, 0,
                margLeft, viewYHight, ptLine);//纵坐标
        canvasBm.drawLine(margLeft,
                0,
                margLeft - getResources().getDimensionPixelSize(R.dimen.dimen4),
...
//                canvasBmTemp.drawText(xValueButtom.get(i), timeBmTemp.getWidth() / 4 * 3, fontHeight, ptTextButtom);
//            } else {
//                canvasBmTemp.drawText(xValueButtom.get(i), timeBmTemp.getWidth() / 2, fontHeight, ptTextButtom);
//            }
//            timeBmTemp = rotateBitmap(timeBmTemp, 180);
//            float withXPosition = (int) (xSaction * (i * 2 + 1) + margLeft - xSaction / 2 - fontHeight / 2);//每一个值横坐标的位置
...
//                canvasBmTemp.drawText(xValueTop.get(i), timeBmTemp.getWidth() / 4 * 3, fontHeight, ptTextTop);
//            } else {
//                canvasBmTemp.drawText(xValueTop.get(i), timeBmTemp.getWidth() / 2, fontHeight, ptTextTop);
//            }
//            float withXPosition = (int) (xSaction * (i * 2 + 1) + margLeft - xSaction / 2);//每一个值横坐标的位置
//            canvasBm.drawBitmap(adjustPhotoRotation(timeBmTemp, 90), withXPosition, 0, new Paint());
//        }
        for (int i = 0; i < xValueTop.size(); i++) {
            Bitmap timeBmTemp = Bitmap.createBitmap((int)xSaction/2*3,(int) margButton, Bitmap.Config.ARGB_8888);
            Canvas canvasBmTemp = new Canvas(timeBmTemp);
            if (xValueTop.get(i).length() < 4){
                canvasBmTemp.drawText(xValueTop.get(i).replace("时",""), timeBmTemp.getWidth() / 2, fontHeight/5*3, ptTextTop);
                canvasBmTemp.drawText("时", timeBmTemp.getWidth() / 2, fontHeight/2*3, ptTextTop);
            }else{
                String tiem=xValueTop.get(i);
                canvasBmTemp.drawText(tiem.substring(0,tiem.indexOf("日")), timeBmTemp.getWidth() / 2, fontHeight/5*3, ptTextTop);
                canvasBmTemp.drawText("日", timeBmTemp.getWidth() / 2, fontHeight/2*3, ptTextTop);
            }

            float withXPosition = (int) (xSaction * (i * 2 + 1) + margLeft - xSaction / 2);//每一个值横坐标的位置
            canvasBm.drawBitmap(timeBmTemp, withXPosition, 0, new Paint());
        }

//        int margBtemp=getResources().getDimensionPixelSize(R.dimen.dimen2);
        for (int i = 0; i < xValueButtom.size(); i++) {
            Bitmap timeBmTemp = Bitmap.createBitmap((int)xSaction/2*3,(int) margButton, Bitmap.Config.ARGB_8888);
            Canvas canvasBmTemp = new Canvas(timeBmTemp);
            if (xValueButtom.get(i).length() < 4){
                canvasBmTemp.drawText(xValueButtom.get(i).replace("时",""), timeBmTemp.getWidth() / 2, timeBmTemp.getHeight() / 2-fontHeight/2, ptTextButtom);
                ...
                canvasBmTemp.drawText(tiem.substring(0,tiem.indexOf("日")), timeBmTemp.getWidth() / 2, timeBmTemp.getHeight() / 2-fontHeight/2, ptTextButtom);
                canvasBmTemp.drawText("日", timeBmTemp.getWidth() / 2, timeBmTemp.getHeight() / 2+fontHeight/2, ptTextButtom);
            }

//          canvasBmTemp.drawText("10", timeBmTemp.getWidth() / 3, fontHeight/5*3, ptTextButtom);
//          canvasBmTemp.drawText("时", timeBmTemp.getWidth() / 2, fontHeight/2*3, ptTextButtom);
            float withXPosition = (int) (xSaction * (i * 2 + 1) + margLeft - xSaction / 2);//每一个值横坐标的位置
            canvasBm.drawBitmap(timeBmTemp, withXPosition, getHeight() - margButton, new Paint());
...
//            数据为空的时候
     ...
     */
    private void setAllXYValue(float yProportion, float xSaction, List<Float> listDataAc, List<Float> listDataPre, float min) {
