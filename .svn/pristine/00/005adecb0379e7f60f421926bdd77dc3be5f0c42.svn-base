...
import com.pcs.ztqtj.control.tool.utils.DateUtil;
import com.pcs.ztqtj.control.tool.utils.ScreenUtil;
...
/**
 * @author chencunxin on 2018/5/11.
...
public class TimeSelector {
    ...
    private final int MAXMINUTE = 12;
    ...
    private int startYear, startMonth, startDay, startHour, startMininute, liveYear, liveMonth, liveDay, liveHour,
            liveMin, endYear, endMonth, endDay, endHour,
            endMininute, minute_workStart, minute_workEnd, hour_workStart, hour_workEnd;
    ...
    public void show() {
        if (startCalendar.getTime().getTime() >= endCalendar.getTime().getTime()) {
            Toast.makeText(context, "开始时间大于结束时间,请重新选择结束时间", Toast.LENGTH_LONG).show();
            return;
        }
...
//        selectedCalender.setTime(endCalendar.getTime());
    ...
    public void setTime( String date) {
        Calendar liveCalendar = Calendar.getInstance();
        liveCalendar.setTime(DateUtil.parse(date, FORMAT_STR));
        selectedCalender.setTime(liveCalendar.getTime());
        liveYear = liveCalendar.get(Calendar.YEAR);
        liveMonth = liveCalendar.get(Calendar.MONTH) + 1;
        liveDay = liveCalendar.get(Calendar.DAY_OF_MONTH);
        liveHour = liveCalendar.get(Calendar.HOUR_OF_DAY);
        liveMin = liveCalendar.get(Calendar.MINUTE);
    }
    ...
    private void loadComponent() {
        ...
        if (liveMonth != endMonth) {
            if (endDay <= liveDay) {
                for (int i = endDay; i <= MaxDay; i++) {
                    day.add(fomatTimeUnit(i));
                }
            } else {
                for (int i = liveDay; i < endDay; i++) {
                    day.add(fomatTimeUnit(i));
                }
            }
        } else {
        ...
        if (liveMonth == endMonth) {
            if (liveDay == endDay) {
                if (liveHour==endHour){
                    for (int i = startMininute; i <= (endMininute / 5); i++) {
                        minute.add(fomatTimeUnit(i * 5));
                    }
                }else{
                    for (int i = startMininute; i <= MAXMINUTE; i++) {
...
//        }
        ...
        for (int i = 0; i < month.size(); i++) {
            if ((liveMonth + "").equals(month.get(i))) {
                month_pv.setSelected(i);
            } else if (("0" + liveMonth).equals(month.get(i))) {
                month_pv.setSelected(i);
            }
        }
//        month_pv.setSelected(1);


        for (int i = 0; i < day.size(); i++) {
            if ((liveDay + "").equals(day.get(i))) {
                day_pv.setSelected(i);
            } else if (("0" + liveDay).equals(day.get(i))) {
                day_pv.setSelected(i);
            }
        }

        for (int i = 0; i < hour.size(); i++) {
            if ((liveHour + "").equals(hour.get(i))) {
                hour_pv.setSelected(i);
            } else if (("0" + liveHour).equals(hour.get(i))) {
                hour_pv.setSelected(i);
            }
        }

        for (int i = 0; i < minute.size(); i++) {
            if ((liveMin + "").equals(minute.get(i))) {
                minute_pv.setSelected(i);
            } else if (("0" + liveMin).equals(minute.get(i))) {
                minute_pv.setSelected(i);
            }
        }
        excuteScroll();
    ...
    private void minuteChange() {
        if ((scrollUnits & SCROLLTYPE.MINUTE.value) == SCROLLTYPE.MINUTE.value) {
            ...
            if (selectedYear == startYear && selectedMonth == startMonth && selectedDay == startDay && selectedHour == startHour) {
                ...
                for (int i = MINMINUTE; i <= endMininute / 5; i++) {
                    minute.add(fomatTimeUnit(i * 5));
                }
